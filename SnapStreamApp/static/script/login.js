...
$(document).ready(function () {
        ...
        success: function (data) {
            if (Array.isArray(data)) {
                data.forEach(function (camera) {
                    ...
                    $('#regCamera').append(new Option(camera, camera));

                    // Add checkboxes
                    $('#cameraList').append(`
                        <label>
                            <input type="checkbox" name="cameras" value="${camera}">
                            ${camera}
                        </label>
                    `);
                });
    ...
    $('#registerForm').submit(function (e) {
        e.preventDefault();
        var selectedCameras = [];
        $('input[name="cameras"]:checked').each(function () {
            selectedCameras.push($(this).val());
            console.log("Selected: " + selectedCameras);
        });

        var formData = $(this).serializeArray();
        formData.push({ name: 'regCamera', value: selectedCameras.join(',') });
    ...
    function validateCameras() {
        ...
        var errorIcon = $('#cameraList').closest('.input-box2').find('.error-icon');
        var errorMessage = $('#cameraErrorMessage');

        if (selectedCameras.length === 0) {
            errorIcon.show();
            errorIcon.attr('title', 'Please select at least one camera.');
            errorMessage.text('Please select at least one camera.').show();
        } else {
            errorIcon.hide();
            errorIcon.removeAttr('title');
            errorMessage.hide();
    ...
    // Adaugă event listener pentru schimbări în checkboxes
    $(document).on('change', 'input[name="cameras"]', function () {
        validateCameras();
    });

    // Rulează validarea la încărcarea paginii
    validateCameras();
});
